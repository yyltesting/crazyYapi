...
const caseModel = require('../models/caselib.js');
...
class baseController {
  ...
  async getProjectRole(id, type) {
    ...
    try {
      ...
      if (type === 'case') {
        let caselib = yapi.getInst(caseModel);
        let caseData = await caselib.get(id);
        ...
        id = demandData.project_id;
      }
      if (type === 'demand') {
        let demand = yapi.getInst(demandModel);
        let demandData = await demand.get(id);
        result.demandData = demandData;
        // 项目创建者相当于 owner
   ...
   * 身份验证
   * @param {*} id type对应的id
   * @param {*} type enum[demand,interface, project, group,case]
