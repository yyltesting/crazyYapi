...
const demandlibModel = require('../models/demand.js')
...
// 连接mysql
const mysqlconnect = require('../utils/database.js');
// 连接redis
const redisconnect = require('../utils/redis.js');
// 连接es
const esconnect = require('../utils/elasticsearch.js');
...
class projectController extends baseController {
  ...
  verifyDomain(domain) {
    ...
    /**
   * mysql连接测试
   * @interface /project/mysqlconnection
   * @method POST
   * @category project
   * @foldnumber 10
   * @param {String} host
  ...
  async mysqlconnection(ctx) {
    try {
      ...
      let result = await mysqlconnect.Connecttion(envid,data);
    ...
    /**
 * redis连接测试
 * @interface /project/redisconnection
 * @method POST
 * @category project
 * @foldnumber 10
 * @param {String} host
 ...
 */
  async redisconnection(ctx) {
    try {
      ...
      delete params.envid;
      let data = params;
      let result = await redisconnect.Connecttion(envid,data);
      ctx.body = yapi.commons.resReturn(result);
    } catch (err) {
      ctx.body = yapi.commons.resReturn(null, 402, err.message);
    }
      ...
      /**
 * es连接测试
 * @interface /project/esconnection
 * @method POST
 * @category project
 * @foldnumber 10
 * @param {String} url
 * @return {Object}
 */
  async esconnection(ctx) {
    try {
      let params = ctx.params;
      let envid = params.envid;
      delete params.envid;
      let data = params;
      let result = await esconnect.createClient(envid,data);
      ctx.body = yapi.commons.resReturn(result);
    } catch (err) {
      ctx.body = yapi.commons.resReturn(null, 402, err.message);
    }
  ...
  async get(ctx) {
    ...
    //默认关闭项目邮件通知
    let data = {"switch_notice":false};
    await this.Model.up(projectId, data);
   ...
   */
   ...
   async colstats(ctx) {
    ...
    let colcase = await interfacecolInst.getcolcase(projectId);
    let getcolcasefail = await interfacecolInst.getcolcasefail(projectId);
    let colcasesuccess=colcase-getcolcasefail;
    ctx.body = yapi.commons.resReturn({col:colcase,colfail:getcolcasefail,colsuccess:colcasesuccess});
  }
    /**
   * 获取项目测试集合失败集合
   * @interface /project/failcol
   * @method GET
   * @category project
   * @foldnumber 10
   * @param {Number} id 项目id，不能为空
   ...
   * @param {Array}  [env[].mysql] mysql
   * @param {Array}  [env[].redis] redis
   * @param {Array}  [env[].es] es
   * @param {Array}  [env[].firebase] firebase
   * @returns {Object}
   * @example
   */
  async upEnv(ctx) {
