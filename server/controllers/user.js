...
class userController extends baseController {
  ...
  async login(ctx) {
    ...
    //解密
    password = yapi.commons.decodeAes(password);
    if (!email) {
   ...
   */
   async forget(ctx) {
    ...
    if (!result) {
      return (ctx.body = yapi.commons.resReturn(null, 400, '该用户不存在'));
    } else{
      //生成随机四位数验证码
      let out = Math.floor(Math.random()*10000); while( out < 1000 );
      //验证码对应邮箱存储数据库
      let data = {
      ...
      try {
        let result = await userInst.upcode(email, data);
  ...
  /**
   * 通过验证码更新密码
   * @interface /user/vcodeuppassword
   * @method POST
   * @category user
   ...
   */
  async Vcodeuppassword(ctx) {
    try {
      let params = ctx.request.body;
      let userInst = yapi.getInst(userModel);
      //解密
      params.password = yapi.commons.decodeAes(params.password);
      if (!params.email) {
        return (ctx.body = yapi.commons.resReturn(null, 400, 'email不能为空'));
      }

      if (!params.vcode) {
        return (ctx.body = yapi.commons.resReturn(null, 400, '验证码不能为空'));
      }
      ...
      let user = await userInst.findByEmail(params.email);
      if (!user) {
        return (ctx.body = yapi.commons.resReturn(null, 400, '该用户不存在'));
      } 

      if(user.vcode==params.vcode){
        ...
        let result = await userInst.update(user._id, data);
        ctx.body = yapi.commons.resReturn(result);
      }else{
        return (ctx.body = yapi.commons.resReturn(null, 400, '验证码不正确'));
      }
    } catch (e) {
    ctx.body = yapi.commons.resReturn(null, 401, e.message);
  }
