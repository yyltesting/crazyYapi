...
module.exports = function renderToHtml(reports,logurl,jobname) {
  let tp = createHtml(reports,logurl,jobname);
  return tp;
};

function createHtml(reports,logurl,jobname) {
  let mdTemplate = ``;
  let left = ``;
  ...
  return createHtml5(left, mdTemplate, reports.message, reports.runTime,reports.colName,reports.startTime,reports.endTime,logurl,jobname);
}

function createHtml5(left, tp, msg, runTime,colName,startTime,endTime,logurl,jobname) {
  let message = ``;
  if (msg.failedNum === 0) {
  ...
  <body class="yapi-run-auto-test">
    <div class="m-header">
      ...
      <a href="#"><h1 class="title">YAPI 测试结果文档 <small>（每次刷新都将是一次新的执行）</small></h1></a>
      <div class="nav">
        <a href="https://github.com/yyltesting/crazy-yapi">YApi</a>
      </div>
    </div>
    <div class="g-doc">
      <div class="menu-left">
      ...
      <h1>YApi ${colName} 测试报告</h1>
      <div class="summary">${message}</div>
      ${tp}
        <footer class="m-footer">
          <p>Build by <a href="https://github.com/yyltesting/crazy-yapi">Tester</a>.</p>
        </footer>
      </div>
    </div>
    <div>
      <p>日志记录服务地址参考：${script}</p>
    </div>
    <script>
      ${script}
    </script>
...
function requestHtml(url, headers, params) {
  ...
  params = escapeHtml(json_format(params));
  let html = ``;
  html += `
  <div>
    <h3>Request</h3>
...
function reponseHtml(res_header, res_body) {
  ...
  res_body = escapeHtml(json_format(res_body, null, '   '));
  let html = ``;
...
function leftHtml(index, name, code) {
  ...
  switch (code) {
    ...
    case 400:
      html = `
      <div class="list-content">
        <a class="list" style="color: orange;" href="#${index}">${name}</a>
        ${codeHtml(code)}
      </div>
      `;
      break;
    case 1:
      html = `
      <div class="list-content">
        <a class="list" style="color: red;" href="#${index}">${name}</a>
        ${codeHtml(code)}
      </div>
      `;
      break;
