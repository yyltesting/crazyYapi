...
class InterfaceNotificationSender {
    ...
    async send() {
        ...
        this.lark.hooks.forEach(async (url) => {
    ...
    async retrievelarkModel() {
        ...
        this.lark = await Model.getByProejctId(this.interfaceModel.project_id)
    }
    ...
    isNotNeedNotify() {
        return !(this.lark && this.lark.hooks && (this.lark.hooks.length > 0));
    }
}
