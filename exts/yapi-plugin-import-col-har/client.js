...
import { unbase64 } from '../../common/utils.js';
...
function postman(importColDataModule) {
  ...
  function run(res) {
    try {
      ...
      let interfaceColData = { apis: [] };
      res = checkInterRepeat.bind(this)(res);
      if (res && res.length) {
        for (let item in res) {
          let data = importHar.bind(this)(res[item]);
          interfaceColData.apis.push(data);
        }
      }
      return interfaceColData;
    } catch (e) {
      console.error(e);
  ...
  function importHar(data, key) {
    ...
    for (let item in key) {
      ...
      if (item === 'req_query') {
        ...
        res[item] = data.request.postData.text;
    ...
    res.req_headers = data.request.headers;
    return res;
  }

  if (!importColDataModule || typeof importColDataModule !== 'object') {
    console.error('obj参数必需是一个对象');
    return null;
  }

  importColDataModule.har = {
    name: 'HAR',
    run: run,
...
module.exports = function() {
  this.bindHook('import_col_data', postman);
};
