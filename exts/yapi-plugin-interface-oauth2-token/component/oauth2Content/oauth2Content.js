  ...
  handleInit(data) {
    ...
    if (request_type) {
      ...
      this.setState({
        ...
        case_pre_script:data.case_pre_script,
        case_post_script:data.case_post_script
      });
    } else {
      this.setState({
        oauth_data: {
  ...
  handleOk = e => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        assignValue.case_post_script = this.state.case_post_script;
        assignValue.case_pre_script = this.state.case_pre_script;
  ...
  validGetTokenUrlValid = async (rule, value, callback) => {
    ...
    try {
      let res = await axios.post('/api/plugin/oauthInterface/url/valid', {
        ...
        form_data,
        data_json,
        case_pre_script:this.state.case_pre_script,
        case_post_script:this.state.case_post_script
      });
        ...
        <div>
          ...
          {request_type === 'POST' && (
            <Collapse bordered={true} style={{ width: '80%',margin:'20px auto' }}>     
              <Panel header="前置/后置js处理器" key="1">
                <div className="project-request">
                  <Form >
                    <FormItem  label="前置处理器:">
                      <AceEditor
                          data={this.state.case_pre_script}
                    ...
                    <FormItem  label="后置处理器">
                      <AceEditor
                          data={this.state.case_post_script}
