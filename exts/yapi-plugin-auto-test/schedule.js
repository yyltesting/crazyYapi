    ...
    async addTestJob(planId, plan) {
        try{
            ...
            let handlerPlan = async (planId, plan, retry) => {
                try {
                    if(plan.plan_fail_retries==retry){
                        //证明已经最后一次
                        isSend = true;
                        url = url.replace("is_send=false", "is_send=true");
                    }else{
                        isSend = false;
                        url = url.replace("is_send=true", "is_send=false");
                    }
                    console.log('retry',retry,url);
                    let result = await axios.get(url, { timeout: 300000 });
                    ...
                    yapi.commons.saveLog({
                        content:plan.plan_name+ e,
                        type: 'project',
                        uid: plan.uid,
                        username: "自动化测试",
                        typeid: plan.project_id
                    });
                }
            }
