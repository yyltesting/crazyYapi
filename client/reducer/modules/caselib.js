...
// Actions
const CASELIB_LIST = 'yapi/caselib/CASELIB_LIST';
...
export default (state = initialState, action) => {
  switch (action.type) {
    case CASELIB_LIST: {
      return {
        ...state,
        caseliblist: action.payload.data.data.list,
...
// Action Creators
export async function caseliblist(data) {
  try {
    const result = await axios.post('/api/caselib/list', data);
    return {
      type: CASELIB_LIST,
      payload: result
    };
