...
export default class InterfaceCaseContent extends Component {
  ...
  async componentWillMount() {
    ...
    let currColId = res.payload.data.data.col_id;
    ...
    //存储对应colid 用于树对应显示
    if(localStorage.getItem('libCase')){
      localStorage.setItem('libCol',this.props.currCase.col_id);
      localStorage.removeItem('libCase');
    }
  }
  ...
  updateCase = async () => {
    ...
    if (this.props.currCase.casename !== casename) {
      ...
      this.props.fetchInterfaceColListchild(this.props.currCase.col_id);
      //
    }
    if (res.data.errcode) {
