...
class VariablesSelect extends Component {
  ...
  async componentDidMount() {
    const { currColId, fetchVariableParamsList, clickValue,col_id } = this.props;
    console.log('props',this.props);
    let result;
    if(currColId===0){
      result = await fetchVariableParamsList(col_id);
    }else{
      result = await fetchVariableParamsList(currColId);
    }
