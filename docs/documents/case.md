...
<img src="./images/autoTestpro.png" />
...
返回 response header 记录了接口运行时长runtime
...
断言函数，详细api可查看 <a target="_blank" href="https://www.chaijs.com/guide/styles/">document</a>

#### 9.Mysql和Redis查询断言

mysqlOpt、redisOpt进行操作数据可直接断言，注意是异步的。也可使用后置js处理器进行context.utils.axios请求修改数据后断言，也可在断言脚本中进行axios请求取出数据后断言

```
var query = 'select userid from habox.user_info where name = "tester";';
assert.rejects(
...
点击服务端测试，出现如下弹窗，用户访问该 url 就可以获取当前测试用例的所有测试结果。可包含所有子集合、子数据驱动；可出现失败用例终止运行；可打开general日志记录
